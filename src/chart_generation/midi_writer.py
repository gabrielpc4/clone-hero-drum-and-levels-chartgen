"""
MIDI Writer: takes an original notes.mid (Harmonix), generates E/M/H reductions for
PART GUITAR and PART DRUMS from Expert and writes new notes.mid.

Strategy:
  - copies all tracks EXCEPT PART GUITAR and PART DRUMS
  - PART GUITAR: replaces E/M/H generated by reducer; preserves Expert + markers
  - PART DRUMS: replaces E/M/H with generated drum charts; preserves Expert,
    cymbal markers (110/111/112), drum fills (120-124), 2x kick (95), animations
"""
...
def _decode_track_abs(track):
    """Returns events from a MidiTrack as (abs_tick, msg) without modification."""
    abs_t = 0; out = []
    for msg in track:
        abs_t += msg.time
...
def _make_guitar_track_from_charts(orig_track, charts_by_diff, replace_diffs=("Easy","Medium","Hard")):
    ...
    # End-of-track needs to stay at the end — we remove any existing EOT and re-add it
...
def write_reduced_midi(input_mid_path: str, output_mid_path: str,
    ...
    """Reads the original notes.mid, generates reductions for the specified PARTs and writes new file."""
...
def diff_midi(orig_path: str, gen_path: str) -> dict:
    """Compares notes.gen vs original in PART GUITAR and PART DRUMS."""
