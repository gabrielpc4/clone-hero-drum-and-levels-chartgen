...
Exception:
- when you explicitly pass `--ref-path` and `--audio-path`, it reads the reference chart only to place the generated drums on the target TPB and uses the song audio only to align the first drum hit
...
If `--ref-path` and `--audio-path` are provided:
